...
const useUpdatingProducts = ()=>{
	...
	useEffect(() => {
		const unSubProducts = onSnapshot(query(collection(db, "products"), orderBy("productName")), async (snapshot) => {
			console.log(...firebaseConsoleBadge, 'Admin Product Snapshot Updated');
			const newProducts = await Promise.all(snapshot.docs.map(fillProductDoc))
...
const ProductElement = ({ product, openEditModal }: { product: ProductInterface, openEditModal: (p: ProductInterface)=>void})=>{
	const item = {
	...
	return (
		<div className="relative">
			<AnimatePresence mode="wait">
				{
					initialLoaded ?
					<motion.div className="grid md:grid-cols-2 2xl:grid-cols-3 gap-x-1 gap-y-1" key="products">
						<AnimatePresence mode="popLayout">
							{products.map(product => <ProductElement key={product.firestoreID} product={product} openEditModal={openEditModal}/>)}
						</AnimatePresence>
						{/* ADD PRODUCT BUTTON */}
						{
							pendingEmpty &&
